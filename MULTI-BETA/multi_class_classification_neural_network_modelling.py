...
class MultiClassClassificationModel:
    ...
    def preprocess_data(self):
        ...
        scaled_features = scaler.fit_transform(self.data[['day_of_year', 'sin_day_of_year', 'cos_day_of_year', 'sin_latitude', 'cos_latitude']])
        scaled_df = pd.DataFrame(scaled_features, columns=['day_of_year_scaled', 'sin_day_of_year_scaled', 'cos_day_of_year_scaled', 'sin_latitude_scaled', 'cos_latitude_scaled'])
        ...
        # kodowanie półkul i pór roku
        hemisphere_encoder = OneHotEncoder(sparse_output=False)
        hemisphere_encoded = hemisphere_encoder.fit_transform(self.data[['hemisphere']])
        hemisphere_labels = hemisphere_encoder.categories_[0]

        season_encoder = OneHotEncoder(sparse_output=False)
        season_encoded = season_encoder.fit_transform(self.data[['season']])
        season_labels = season_encoder.categories_[0]

        # dodanie kodowanych półkul i pór roku do danych
        hemisphere_df = pd.DataFrame(hemisphere_encoded, columns=[f"hemisphere{label}" for label in hemisphere_labels])
        season_df = pd.DataFrame(season_encoded, columns=[f"season_{label}" for label in season_labels])
        self.data = pd.concat([self.data, hemisphere_df, season_df], axis=1)
        ...
        with open("../output/hemisphere_encoder.pkl", 'wb') as file:
            pickle.dump(hemisphere_encoder, file)
        print("hemisphere encoder zapisano do pliku: output/hemisphere_encoder.pkl")

        with open("../output/season_encoder.pkl", 'wb') as file:
            pickle.dump(season_encoder, file)
        print("season encoder zapisano do pliku: output/season_encoder.pkl")
    ...
    def build_model(self, input_shape, output_shape):
        ...
        model.add(Dense(256, input_shape=(input_shape,), activation='relu', kernel_regularizer=l2(0.01),
