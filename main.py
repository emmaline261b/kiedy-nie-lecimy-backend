...
print('kolumny w pliku: \n', wczytany_plik.columns)
print('rozmiar pliku', wczytany_plik.shape)
...
dane['Duration'] = (dane['End Date'] - dane['Start Date']).dt.days
...
szereg_czasowy_katastrofy_df['Days Since Start'] = (
        szereg_czasowy_katastrofy_df['Date'] - szereg_czasowy_katastrofy_df['Start Date']
).dt.days

szereg_czasowy_katastrofy_df['Days Till End'] = (
        szereg_czasowy_katastrofy_df['End Date'] - szereg_czasowy_katastrofy_df['Date']
).dt.days

# Usuwanie kolumn 'Start Date' i 'End Date'
szereg_czasowy_katastrofy_df = szereg_czasowy_katastrofy_df.drop(columns=['Start Date', 'End Date'])

szereg_czasowy_katastrofy_df.head()

countries = szereg_czasowy_katastrofy_df['Country'].unique()

# Pobieranie unikalnych map dla Subregion i Region
country_to_subregion = \
    szereg_czasowy_katastrofy_df.dropna(subset=['Subregion']).drop_duplicates('Country').set_index('Country')[
        'Subregion']
country_to_region = \
    szereg_czasowy_katastrofy_df.dropna(subset=['Region']).drop_duplicates('Country').set_index('Country')['Region']
...
combined_df = complete_df.drop(['Duration', 'Days Since Start', 'Days Till End', 'Region'], axis=1)
...
for i in range(len(features_matrix) - sequence_length):
    X_lstm_updated.append(features_matrix[i:i + sequence_length])  # Dane wejściowe: ostatnie 20 dni
    y_lstm_updated.append(target_array[i + sequence_length])  # Wynik: czy wystąpi katastrofa
